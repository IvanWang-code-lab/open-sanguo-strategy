...
export const resolveBattle = (state: GameState, attackerCityId: string, defenderCityId: string, options: ResolveBattleOptions = {}) => {
  ...
  if (attackerCity.actedThisTurn) {
    return { state, report: undefined, logs: [`${attackerCity.name}本回合已经行动，无法再次出征`] };
  }
  ...
  const sentTroops = totalTroops(assignedTroops) > 0 ? assignedTroops : scaleTroops(attackerCity.troops, 0.55);
